...
## This directory has an `.envrc`, and it matters for diagnosis

`FLEETCOM/.envrc` sources `auditboard-dev-env/.envrc` and holds no values of its
own. Without it, a shell sitting in FLEETCOM has **none** of AuditBoard's real
configuration, and the same question gets a different answer here than it does
where the service runs.

That is not hypothetical. `CASCADE_JWT_SECRET` is generated by
`fleetcom-onboard.sh`, written to `auditboard-dev-env/.envrc`, and paired with
`JWT_AUTH_SHARED_SECRET` in `cascade/.env` — the two halves of local Cascade
SSO. AuditBoard reads it only as an environment variable. Reading
`auditboard-backend/config/default.mjs` from a FLEETCOM shell shows
`cascade.jwt_secret: ''` and invites the conclusion that it is unset. It is set,
paired, and merely absent from that shell. A session made exactly that mistake
and reported a non-existent blocker.

So: **if you are about to conclude something is unconfigured, check whether you
are in a direnv-loaded shell first.** `direnv exec . <cmd>` from FLEETCOM, or
just let the `.envrc` load.

Edit either file and direnv needs `direnv allow` again, or every `direnv exec`
fails.
